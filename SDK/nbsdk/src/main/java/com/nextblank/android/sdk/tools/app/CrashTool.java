package com.nextblank.android.sdk.tools.app;
 ...
 * 崩溃相关工具类
 */
public final class CrashTool implements UncaughtExceptionHandler {
    private static final Object lock = new Object();
    private static volatile CrashTool instance;
    ...
    public boolean init() {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            File baseCache = Tools.app().getExternalCacheDir();
            if (baseCache == null) return false;
            crashDir = baseCache.getPath() + File.separator + "crash" + File.separator;
        } else {
            File baseCache = Tools.app().getCacheDir();
            if (baseCache == null) return false;
            crashDir = baseCache.getPath() + File.separator + "crash" + File.separator;
        }
        try {
            PackageInfo pi = Tools.app().getPackageManager().getPackageInfo(Tools.app().getPackageName(), 0);
